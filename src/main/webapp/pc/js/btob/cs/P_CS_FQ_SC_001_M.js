...
var mController = $.extend(new $.CommonObj(),{
	...
	eventInit:function() {
        ...
        $(document).on('click', '#m_sch_txt_submit, #m-search-panel > li a', function() {
            _this.search();
        });

        $(document).on('click', '#m-list-panel li a', function() {
	...
	onCreate:function() {
        ...
        if(pageReqParams.ctgryCd != '') {
            $('#m-search-panel li.sch_cate a').trigger('click');
            var ctgryInfo = this.setCtgryInfo(pageReqParams.ctgryCd);
            if(ctgryInfo.lCtgryCd != null) {
            	setTimeout(function() {
                	$('#m-search-panel div[data-ctgry="L"] li a[data-ctgry-cd="' + ctgryInfo.lCtgryCd + '"]').trigger('click');
                }, 100);
            }
            if(ctgryInfo.mCtgryCd != null) {
                setTimeout(function() {
                    $('#m-search-panel div[data-ctgry="M"] li a[data-ctgry-cd="' + ctgryInfo.mCtgryCd + '"]').trigger('click');
                }, 200);
            }
            if(ctgryInfo.sCtgryCd != null) {
                setTimeout(function() {
                    $('#m-search-panel div[data-ctgry="S"] li a[data-ctgry-cd="' + ctgryInfo.sCtgryCd + '"]').trigger('click');
                }, 300);
            }
        } else {
            this.search();
    ...
    hideComboCtgry: function(ctgryLvl) {
        if(!ctgryLvl || ctgryLvl <= 1)$(this.wrapCls + ' div[data-ctgry="L"]').hide().find('ul').empty().prev().text('전체');
        if(!ctgryLvl || ctgryLvl <= 2)$(this.wrapCls + ' div[data-ctgry="M"]').hide().find('ul').empty().prev().text('선택');
        if(!ctgryLvl || ctgryLvl <= 3)$(this.wrapCls + ' div[data-ctgry="S"]').hide().find('ul').empty().prev().text('선택');
    },
    ...
    setComboLCtgry: function() {
        ...
        elemList.empty();
        var html = [];
        html.push('<li class="on"><a href="#m_sel02" data-ctgry-cd="">전체</a></li>');
        listLCtgry.forEach(function(item) {
        	if(item.ctgryNm != '5G') {
	        	html.push('<li><a href="#m_sel02" data-ctgry-cd="' + item.gCtgryCd + '_' + item.lCtgryCd + '">' + item.ctgryNm + '</a></li>');
        	}
        });
    ...
    setComboMCtgry: function() {
        ...
        if(listMCtgry.length > 0) {
            ...
            elemList.empty();
            var html = [];
            html.push('<li class="on"><a href="#m_sel03" data-ctgry-cd="">선택</a></li>');
            listMCtgry.forEach(function(item) {
                html.push('<li><a href="#m_sel03" data-ctgry-cd="' + item.gCtgryCd + '_' + item.lCtgryCd + '_' + item.mCtgryCd + '">' + item.ctgryNm + '</a></li>');
            });
            elemList.append(html.join(''));
    ...
    setComboSCtgry: function() {
        ...
        if(listSCtgry.length > 0) {
            ...
            elemList.empty();
            var html = [];
            html.push('<li class="on"><a href="#m_sel04" data-ctgry-cd="">선택</a></li>');
            listSCtgry.forEach(function(item) {
            	if(item.ctgryCd != 'PD_CC_IT_001') {
            		html.push('<li><a href="#m_sel04" data-ctgry-cd="' + item.ctgryCd + '">' + item.ctgryNm + '</a></li>');
            	}
            });
            elemList.append(html.join(''));
    ...
    getSearchParams: function() {
        var rtnParams = {};
        if($('#m-search-panel li.sch_txt').hasClass('active')) {  // 검색으로 찾기
            rtnParams.searchText = $('#m_sch_txt').val().trim();
        } else {    // 카테고리로 찾기
    ...
    searchPaging: function() {
        ...
        this.ajaxSendCall('/bbs/listFaq.json', params, function(result) {
            if(result.RES.returnCode == '01') {
                ...
                $('#m-list-panel .faq_count').html('총 <b>' + result.cntFaq + '</b>건');
