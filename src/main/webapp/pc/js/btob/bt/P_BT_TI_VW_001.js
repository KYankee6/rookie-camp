...
var controller = $.extend(new $.CommonObj(),{
    ...
    detail: null,
	eventInit:function() {
		$(document).on('click', '.kt_pc .bnr_banner_qna', function() {
            $("select[name=QUESTIONTYPE]").val('DX Story').prop("selected", true); 
        });
	}, 
	onCreate:function() {
		if(mode_chk == 'pc'){	// pc/m 중복으로 실행되는 것을 막기 위해 접속 환경이 pc 일 경우에만 함수 실행. 
			this.makeSubTab("A");
			this.getDetail();
		}
	},
    ...
    dispDetail: function() {
        var _this = this;
        document.cookie = "bbsId=" + this.detail.bbsId + ";"
        document.cookie = "title=" + this.detail.title + ";"
        ...
        if(this.nvl(this.detail.bbsTp, "") == "") {
            $('.kt_pc .bnr_view .bnr_view_title .bedge').remove();
        } else { 
            $('.kt_pc .bnr_view .bnr_view_title .bedge span').text(this.getBbsTpNm(this.detail.bbsTp));
        }
        // title
        //$('.kt_pc .bnr_view .bnr_view_title .title').text(this.detail.title);
        // 등록일
        var regDt = new $.Utils.datetime(this.detail.regDttm).getDate('yyyy.mm.dd');
        var regDtHtml = [];
        $('.kt_pc .bnr_view .bnr_view_title dl').empty();
        regDtHtml.push('<dt>등록일자</dt>');
        regDtHtml.push('<dd>' + regDt + '</dd>');
        $('.kt_pc .bnr_view .bnr_view_title dl').append(regDtHtml.join(''));
        // content
        //$('.kt_pc .bnr_view .bnr_view_cont').empty().html(this.detail.content);
        // 리포트,첨부파일 다운로드
        $('.kt_pc .bnr_view .bnr_view_download').empty();
        if(listReport.length == 0 && listAttFile.length == 0) {
            $('.kt_pc .bnr_view .bnr_view_download').remove();
        } else {
            var fileHtml = [];
            ...
            $('.kt_pc .bnr_view .bnr_view_download').append(fileHtml);
        }
        // 관련상품군 시작
        $('.j_banner_slide02').slick('unslick');
        $('.kt_pc .banner_list02').empty();
        var html = [];
        ...
        $('.kt_pc .banner_list02').append(html.join(''));
        ...
        // 검색태그
        $('.kt_pc .area_tags').empty();
        $('.kt_pc .area_tags').append(this.getHashtagHtml(this.detail.arrHashTag, this.detail.bbsTp, '<li><a href="#" title="#hashTag#" data-link-hashtag="#hashTag#">#hashTag#</a></li>'));
        // 이전글다음글
        $('.kt_pc .bnr_prev_next .inner').empty();
        html = []
        ...
        $('.kt_pc .bnr_prev_next .inner').append(html.join(''));
